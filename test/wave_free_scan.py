...
wave_free_scan_pass_program = count.device.load_program("wave_free_scan.slang", ["wave_free_scan_pass"])
wave_free_scan_pass_kernel = count.device.create_compute_kernel(wave_free_scan_pass_program)
...
wave_free_scan_add_pass_program = count.device.load_program("wave_free_scan.slang", ["wave_free_scan_add_pass"])
wave_free_scan_add_pass_kernel = count.device.create_compute_kernel(wave_free_scan_add_pass_program)

wave_free_reduce_table_buffer = count.device.create_buffer(
    usage=spy.BufferUsage.shader_resource |spy.BufferUsage.unordered_access,
    data=count.original_reduce_table_result,
    label="wave_free_reduce_table_buffer"
)
wave_free_sum_table_buffer = count.device.create_buffer(
    usage=spy.BufferUsage.shader_resource |spy.BufferUsage.unordered_access,
    data=count.original_sum_table_result,
    label="wave_free_sum_table_buffer"
)
...
with command_encoder.begin_compute_pass() as pass_encoder:
    ...
    cursor["scan_src"] = wave_free_reduce_table_buffer
    cursor["scan_dst"] = wave_free_reduce_table_buffer
    cursor["scan_scratch"] = wave_free_reduce_table_buffer # just put it for generating a bind descriptor
    pass_encoder.dispatch_compute([1, 1, 1])
    ...
    cursor["scan_src"] = wave_free_sum_table_buffer
    cursor["scan_dst"] = wave_free_sum_table_buffer
    cursor["scan_scratch"] = wave_free_reduce_table_buffer
    pass_encoder.dispatch_compute([count.config.num_reduce_threadgroups_to_run, 1, 1])
...
wave_free_scan_reduce_table_result = wave_free_reduce_table_buffer.to_numpy().view(np.uint32).reshape(-1, count.config.num_reduce_threadgroup_per_bin)
assert(np.array_equal(wave_free_scan_reduce_table_result, expected_scan_reduce_table))

wave_free_scan_sum_table_result = wave_free_sum_table_buffer.to_numpy().view(np.uint32).reshape(count.Const.SORT_BIN_COUNT, -1)
assert(np.array_equal(wave_free_scan_sum_table_result, expected_scan_sum_table))