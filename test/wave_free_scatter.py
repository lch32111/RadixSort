...
wave_free_scatter_pass_program = device.load_program("wave_free_scatter.slang", ["wave_free_scatter_pass"])
wave_free_scatter_pass_kernel = device.create_compute_kernel(wave_free_scatter_pass_program)
...
wave_free_out_buffer = device.create_buffer(
    usage=spy.BufferUsage.unordered_access,
    data=np.zeros((scan.count.NUM_KEYS), dtype=np.uint32)
)
...
with command_encoder.begin_compute_pass() as pass_encoder:
    ...
    cursor["src_buffer"] = scan.count.keys_buffer
    cursor["dst_buffer"] = wave_free_out_buffer
    cursor["sum_table"] = scan.count.sum_table_buffer
...
wave_free_out_buffer_result = wave_free_out_buffer.to_numpy().view(np.uint32)
print(wave_free_out_buffer_result)
assert np.array_equal(wave_free_out_buffer_result, sorted_keys)
